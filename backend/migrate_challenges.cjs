...
async function migrate() {
    ...
    try {
        ...
        console.log('Connected to Postgres DB for Challenges migration.');
        ...
        await client.query(`
            CREATE TABLE IF NOT EXISTS challenges (
                ...
                status VARCHAR(30) DEFAULT 'open', -- 'open', 'room_setup', 'ongoing', 'claimed', 'disputed', 'completed', 'cancelled'
                room_id VARCHAR(50),
            ...
            END $$;
        `);
        console.log('✅ challenges table & realtime configured.');

        // 2. Atomic Function: p2p_create_challenge
        await client.query(`
            ...
            BEGIN
                ...
                -- 3. Calculate deductions (Bonus first)
                ...
                -- Calculate pot: Total wager = p_stake * 2. Winner gets 90%, Platform retains 10%
        ...
        `);
        console.log('✅ p2p_create_challenge function created.');

        // 3. Atomic Function: p2p_cancel_challenge
        await client.query(`
            ...
            $$ LANGUAGE plpgsql SECURITY DEFINER;
        `);
        console.log('✅ p2p_cancel_challenge function created.');

        // 4. Atomic Function: p2p_accept_challenge
        await client.query(`
            ...
            BEGIN
                ...
                -- Update challenge to room_setup
                UPDATE challenges 
            ...
            $$ LANGUAGE plpgsql SECURITY DEFINER;
        `);
        console.log('✅ p2p_accept_challenge function created.');

        // 5. Atomic Function: p2p_submit_room
        await client.query(`
            ...
            $$ LANGUAGE plpgsql SECURITY DEFINER;
        `);
        console.log('✅ p2p_submit_room function created.');

        // 6. Atomic Function: p2p_claim_victory
        await client.query(`
            ...
            $$ LANGUAGE plpgsql SECURITY DEFINER;
        `);
        console.log('✅ p2p_claim_victory function created.');

        // 7. Atomic Function: p2p_confirm_loss (Instant Release)
        await client.query(`
            ...
            $$ LANGUAGE plpgsql SECURITY DEFINER;
        `);
        console.log('✅ p2p_confirm_loss function created.');

        // 8. Atomic Function: p2p_oppose_claim
        await client.query(`
            ...
            $$ LANGUAGE plpgsql SECURITY DEFINER;
        `);
        console.log('✅ p2p_oppose_claim function created.');

        // 9. Atomic Function: p2p_report_issue
        await client.query(`
            ...
            $$ LANGUAGE plpgsql SECURITY DEFINER;
        `);
        console.log('✅ p2p_report_issue function created.');

        // 10. Atomic Function: p2p_auto_settle_expired
        await client.query(`
            ...
            $$ LANGUAGE plpgsql SECURITY DEFINER;
        `);
        console.log('✅ p2p_auto_settle_expired function created.');

        // 11. Atomic Function: p2p_admin_resolve
        await client.query(`
            CREATE OR REPLACE FUNCTION p2p_admin_resolve(
                p_challenge_id UUID,
                p_resolution TEXT, -- 'award_host', 'award_opponent', 'refund_both'
            ...
            BEGIN
                ...
                -- Optional Penalty (10 Tk fee from loser to winner for false dispute)
            ...
            $$ LANGUAGE plpgsql SECURITY DEFINER;
        `);
        console.log('✅ p2p_admin_resolve function created.');

        await client.end();
        console.log('🎉 All challenges migration steps executed successfully!');
    } catch (err) {
